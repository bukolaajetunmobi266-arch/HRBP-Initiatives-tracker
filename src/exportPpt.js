...
// Converts action items into a flat list with the correct EFFECTIVE status
// per row — since a shared item's own .status field is meaningless (real
// status lives per-person in actionStatuses), this must be resolved before
// any counting happens, or shared items would report wrong numbers.
function effectiveActionRows(actions, actionStatuses, mode, targetUserId) {
  if (mode === 'admin') {
    return actions.flatMap((a) => {
      if (!a.shared) return [{ status: a.status, due_date: a.due_date }]
      const rows = actionStatuses.filter((s) => s.action_id === a.id)
      return rows.map((r) => ({ status: r.status, due_date: a.due_date }))
    })
  }
  return actions.map((a) => {
    if (!a.shared) return { status: a.status, due_date: a.due_date }
    const row = actionStatuses.find((s) => s.action_id === a.id && s.user_id === targetUserId)
...
export async function generateCpoReviewPack({ deliverables, actions, actionStatuses, profiles, ownerName, winsFrom, winsTo }) {
  ...
  addSnapshotSlide(pres, 'Overall snapshot', deliverables, effectiveActionRows(actions, actionStatuses, 'admin'))
...
export async function generatePersonalReviewPack({ name, deliverables, actions, actionStatuses, myUserId, winsFrom, winsTo }) {
  ...
  addSnapshotSlide(pres, 'My snapshot', deliverables, effectiveActionRows(actions, actionStatuses, 'personal', myUserId))
