...
export function parseImportFile(arrayBuffer, profiles) {
  ...
  if (wb.SheetNames.includes('Action Items')) {
    ...
    rows.forEach((row, i) => {
      ...
      const raisedIn = String(row['Raised In'] || '').trim()
      // "HRBPs" (or similar) in the Owner column means this applies to
      // everyone — each HRBP gets their own independent status row.
      if (['hrbps', 'all hrbps', 'shared', 'all'].includes(ownerRaw.toLowerCase())) {
        result.actions.push({ title, raised_in: raisedIn, shared: true, due_date: dueDate, status, comment })
        return
      }
      const owner = matchOwner(ownerRaw, profiles)
      if (!owner) {
        result.actionErrors.push(`Row ${rowNum}: "${title}" — owner "${ownerRaw}" doesn't match any HRBP name. Row skipped.`)
        return
      }
      result.actions.push({ title, raised_in: raisedIn, shared: false, owner_id: owner.id, due_date: dueDate, status, comment })
    })
  }
