                ...
                {isAdmin ? (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {profiles.filter((p) => p.role !== 'admin' && rows.some((s) => s.user_id === p.id)).map((p) => {
                      ...
                      return (
                        <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, padding: '2px 4px 2px 8px', borderRadius: 999, background: `var(--${role}-bg)`, color: `var(--${role}-tx)`, whiteSpace: 'nowrap' }}>
                          {overdue && '⚠ '}{p.full_name.split(' ')[0]}:
                          <select
                            value={st}
                            onChange={(e) => onAdminStatusChange(a.id, p.id, e.target.value)}
                            style={{ fontSize: 11, border: 'none', background: 'transparent', color: `var(--${role}-tx)`, padding: '2px 4px', cursor: 'pointer' }}
                          >
                            {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
                          </select>
                        </label>
