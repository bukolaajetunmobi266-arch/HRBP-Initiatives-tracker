...
function Dashboard({ session, theme, setTheme }) {
  ...
  const expandedActions = (isAdmin
    ? keyActions.flatMap((a) => {
        if (!a.shared) return [{ ...a, _effStatus: a.status, _owner: a.owner_id }]
        const rows = actionStatuses.filter((s) => s.action_id === a.id)
        return rows.length ? rows.map((r) => ({ ...a, _effStatus: r.status, _owner: r.user_id })) : [{ ...a, _effStatus: 'Not Started', _owner: null }]
      })
    : keyActions
        .filter((a) => a.shared || a.owner_id === userId)
  ...
  const counts = useMemo(() => {
    ...
    kpiSource.forEach((d) => { c[d.status] = (c[d.status] || 0) + 1; if (isOverdue(d)) c.Overdue++ })
    return c
  }, [kpiSource])
  const actionCounts = useMemo(() => {
    const c = { 'Not Started': 0, 'In Progress': 0, Completed: 0, Overdue: 0 }
    expandedActions.forEach((a) => {
      c[a._effStatus] = (c[a._effStatus] || 0) + 1
      if (isActionOverdue(a, a._effStatus)) c.Overdue++
    })
    return c
  }, [expandedActions])
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg0)', color: 'var(--tx1)' }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '1.5rem 1rem' }}>
        <KpiStrip counts={view === 'actions' ? actionCounts : counts} total={view === 'actions' ? expandedActions.length : kpiSource.length} label={view === 'actions' ? 'Action Items' : 'Deliverables'} />
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 10 }}>
              <button onClick={() => setShowPpt(isAdmin ? 'cpo' : 'personal')} style={{ fontSize: 13, background: 'var(--acc-fill)', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 14px', cursor: 'pointer' }}>
                {isAdmin ? 'Generate CPO Review Pack (PPT)' : 'Generate my update pack (PPT)'}
              </button>
            </div>
            <SummaryView
              ...
              sharedWaiting={keyActions.filter((a) => a.shared && myActionStatus(a) !== 'Completed')}
            />
          </>
        )}
      ...
      {showExport && (
        <ExportDialog
          ...
          onExport={(useFiltered, includeActions) => {
            ...
            exportToExcel({ deliverables: rows, keyActions: isAdmin ? keyActions : keyActions.filter((a) => a.shared || a.owner_id === userId), profiles, includeActions })
            setShowExport(false)
          }}
        />
      )}
          ...
          onGenerate={async (from, to) => {
            ...
            try {
              if (showPpt === 'cpo') {
                ...
                const mineActions = keyActions.filter((a) => a.shared || a.owner_id === userId)
...
function KpiStrip({ counts, total, label }) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ fontSize: 11, color: 'var(--txm)', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 0.3 }}>{label}</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {items.map(([l, val]) => (
          <div key={l} style={{ background: 'var(--bg1)', borderRadius: 10, padding: '12px 16px' }}>
            <p style={{ fontSize: 12, color: 'var(--tx2)', margin: '0 0 4px' }}>{l}</p>
...
function SummaryView({ deliverables, profiles, expandedActions, isAdmin, dueThisWeek, sharedWaiting }) {
    ...
    <div>
      ...
      {isAdmin && (
        <div style={{ background: 'var(--bg2)', border: '0.5px solid var(--bd)', borderRadius: 12, padding: 16, marginTop: 16, marginBottom: 4 }}>
          <p style={{ fontSize: 13, color: 'var(--tx2)', margin: '0 0 12px' }}>Completion by HRBP</p>
          {rows.map((r) => (
...
function ActionsView({ keyActions, actionStatuses, profiles, isAdmin, ownerName, myId, onOpen, onAdd, onDelete, onMyStatusChange, onExport, onImport }) {
  const visible = isAdmin ? keyActions : keyActions.filter((a) => a.shared || a.owner_id === myId)
  ...
  const single = visible.filter((a) => !a.shared)
  const shared = visible.filter((a) => a.shared)
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginBottom: 10 }}>
        <button onClick={onImport} style={btnStyle()}>Import from Excel</button>
      ...
      {single.length > 0 && (
        <div style={{ border: '0.5px solid var(--bd)', borderRadius: 10, overflow: 'auto', marginBottom: shared.length ? 20 : 0 }}>
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse', minWidth: 760 }}>
            ...
            <tbody>
              {single.map((a) => {
                const overdue = a.due_date && a.status !== 'Completed' && a.due_date < todayISO()
                return (
                  <tr key={a.id} onClick={() => onOpen(a.id)} style={{ borderTop: '0.5px solid var(--bd)', cursor: 'pointer' }}>
                    ...
                    <td style={{ padding: '8px 12px' }}><StatusBadge status={a.status} overdue={overdue} /></td>
      ...
      {shared.length > 0 && (
        <div>
          <p style={{ fontSize: 12, fontWeight: 500, color: 'var(--tx2)', margin: '0 0 8px' }}>Shared across HRBPs</p>
          {shared.map((a) => {
            const rows = actionStatuses.filter((s) => s.action_id === a.id)
            const myRow = rows.find((s) => s.user_id === myId)
            const myStatus = myRow ? myRow.status : 'Not Started'
            const completedCount = rows.filter((s) => s.status === 'Completed').length
            const totalCount = isAdmin ? (rows.length || profiles.filter((p) => p.role !== 'admin').length) : rows.length
            return (
              <div key={a.id} style={{ border: '0.5px solid var(--acc-tx)', borderRadius: 10, padding: '12px 16px', background: 'var(--bg2)', marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                  <div>
                    <p style={{ fontSize: 13, margin: '0 0 2px' }} onClick={() => onOpen(a.id)}>{a.title}</p>
                    <p style={{ fontSize: 11, color: 'var(--txm)', margin: 0 }}>{a.raised_in} {a.due_date ? `· Due ${fmtDate(a.due_date)}` : ''}</p>
                  </div>
                  {isAdmin && <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--acc-tx)', whiteSpace: 'nowrap' }}>{completedCount} of {totalCount} complete</span>}
                  {isAdmin && <button onClick={() => onDelete(a.id)} style={{ fontSize: 11, color: 'var(--dgr-tx)', background: 'none', border: 'none', cursor: 'pointer' }}>Delete</button>}
                </div>
                {isAdmin ? (
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {profiles.filter((p) => p.role !== 'admin').map((p) => {
                      const row = rows.find((s) => s.user_id === p.id)
                      const st = row ? row.status : 'Not Started'
                      const overdue = a.due_date && st !== 'Completed' && a.due_date < todayISO()
                      const role = overdue ? 'dgr' : st === 'Completed' ? 'suc' : st === 'In Progress' ? 'wrn' : 'neu'
                      return (
                        <span key={p.id} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 999, background: `var(--${role}-bg)`, color: `var(--${role}-tx)`, whiteSpace: 'nowrap' }}>
                          {overdue && '⚠ '}{p.full_name.split(' ')[0]}: {st}
                        </span>
                      )
                    })}
                  </div>
                ) : (
                  <select value={myStatus} onChange={(e) => onMyStatusChange(a.id, e.target.value)} style={{ fontSize: 12, padding: '6px 10px', borderRadius: 8, border: '0.5px solid var(--bds)', background: 'var(--bg2)', color: 'var(--tx1)' }}>
                    {STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
                  </select>
                )}
              </div>
...
function PptDialog({ scope, onCancel, onGenerate, busy }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 50 }}>
      <div style={{ background: 'var(--bg2)', color: 'var(--tx1)', borderRadius: 12, width: '100%', maxWidth: 380, padding: 20 }}>
        <p style={{ fontSize: 15, fontWeight: 500, margin: '0 0 4px' }}>{scope === 'cpo' ? 'CPO Review Pack' : 'My Update Pack'}</p>
